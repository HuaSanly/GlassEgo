...
def run_teacher_forced_vis(
    model: FlowMatchingModel, unit_dir: str, out_dir: str,
    ...
    for i in range(len(frame_dirs)):
        ...
        fd = frame_dirs[i]
        json_path_curr = os.path.join(all_data_dir, fd, "training_data.json")
        d_t = read_json(json_path_curr)
        ...
        x_rgb = dummy_ds._load_image_tensor(json_path_curr, 0).unsqueeze(0).to(device)
        ...
        raw_rgb_bgr = safe_imread_rgb(raw_rgb_path, image_size[0], image_size[1])
        ...
        for h_idx, side in enumerate(hand_sides):
            ...
            for k in range(1, pred_horizon + 1):
                dk = read_json(dummy_ds._get_future_json_path(json_path_curr, k))
                if dk:
    ...
    if make_video and vis_frames:
        create_video_from_frames(
            vis_frames,
            os.path.join(out_dir, "evaluation_vis.mp4"),
            fps=video_fps,
            export_gif=True,
            ratio=10,
        )
