...
class FlowMatchingDataloader(Dataset):
    ...
    def __init__(
        ...
        self._frame_sequences: Dict[str, List[str]] = {}
        self._frame_positions: Dict[str, int] = {}
        self._build_index()

        print(f"[FlowMatchingDataloader] Built {len(self.samples)} samples.")
        print("  - Hand Source  : entities.hands")
    ...
    def _build_index(self):
        ...
        for spec in self.sessions:
            all_data_dir = os.path.abspath(
                os.path.join(os.path.expanduser(spec.unit_dir), "preprocess", "all_data")
            )
            ...
            json_paths = [
                os.path.join(all_data_dir, frame_name, JSON_NAME)
                for frame_name in frame_names
                if os.path.isfile(os.path.join(all_data_dir, frame_name, JSON_NAME))
            ]
            self._frame_sequences[all_data_dir] = json_paths
            self._frame_positions.update(
                {json_path: position for position, json_path in enumerate(json_paths)}
            )
    ...
    def _frame_has_required_fields(self, d: dict) -> bool:
        ...
        hands = d.get("entities", {}).get("hands", {})
        if hands is None:
    ...
    def _get_future_json_path(self, json_path: str, step: int) -> str:
        ...
        position = self._frame_positions.get(json_path)
        if position is None:
            return ""
        future_position = position + step
        sequence = self._frame_sequences.get(all_data_dir, [])
        if future_position < 0 or future_position >= len(sequence):
            return ""
        return sequence[future_position]
    ...
    def _build_targets(self, json_path0: str, T_w2ref_base: np.ndarray, idx: int, stride: int, d0: dict) -> Dict[str, torch.Tensor]:
        ...
        for k in range(1, K + 1):
            ...
            hands = dk.get("entities", {}).get("hands", {})
            if hands is None: hands = {}  # null means method not generated
    ...
    # Images Loader
    # ---------------------
    def _load_image_tensor(self, json_path: str, idx: int) -> torch.Tensor:
        if not self.img_name:
            return torch.zeros((3, self.H, self.W), dtype=torch.float32)

        img_path = os.path.join(os.path.dirname(json_path), self.img_name)
        rgb = cv2.cvtColor(
            safe_imread_rgb(img_path, self.H, self.W),
            cv2.COLOR_BGR2RGB,
        )
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        image_json_path = json_path1 if alpha > 0.5 else json_path0

        # 3. Load Visual Representation
        x_rgb = self._load_image_tensor(image_json_path, idx)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--unit_dir", type=str, required=True)
    parser.add_argument("--dual_hand", action="store_true")
    args = parser.parse_args()

    ds = FlowMatchingDataloader(
        sessions=[UnitSession(unit_dir=args.unit_dir)],
...
# python -m training.FlowMatchingDataloader --unit_dir "./data/serve_bread/unit_001"
