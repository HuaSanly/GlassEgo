...
class FlowMatchingModel(nn.Module):
    ...
    def compute_loss(
        ...
        w_time = self.horizon_weights.to(v_pred.device).view(1, -1, 1).float() #时间步权重
        w_p, w_r, w_g = (weights.get(k, 1.0) for k in ["w_pos", "w_rot", "w_g"]) if weights else (1.0, 1.0, 1.0) #位置、旋转、抓取的权重系数
        w_flow = weights.get("w_flow", 1.0) if weights else 1.0 #flow的权重
